...
npx cspell@${cspell_version:1} . --dot --no-progress --no-summary --unique --words-only --no-exit-code --exclude "**/.infracost/**" --exclude "**/.terraform/**" --exclude ".git/**" --exclude ".idea/**" --exclude "*.tfstate.*" --exclude "$DICTIONARIES_PATH/**" | sort --ignore-case --unique > "$MISSPELLED_WORDS_PATH"
